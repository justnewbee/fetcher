...
describe('fetcherInterceptorLogin', () => {
  ...
  test('will auto login', async () => {
    ...
    expect(needLogin).toHaveBeenCalledTimes(1);
  ...
  test('doLogin is singleton', async () => {
    ...
    expect(needLogin).toHaveBeenCalledTimes(3);
    expect(listenInterceptorLoginCallback).toHaveBeenCalledTimes(1); // message 会被触发
    expect(doLogin).toHaveBeenCalledTimes(1);
  });
  ...
  test('error with no code will not doLogin', async () => {
    ...
    expect(needLogin).toHaveBeenCalledTimes(2);
