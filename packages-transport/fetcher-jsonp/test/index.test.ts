...
describe('fetcherJsonp', () => {
    ...
    vi.spyOn(doc, 'createElement').mockImplementation((tagName: string) => {
      ...
      Object.defineProperty(el, 'src', {
        ...
        set: (value: string) => {
          ...
          const match = /[?&][^=&]+=([^&]+)$/.exec(value) as unknown as null | [string, string];
  ...
  test('404 will trigger network error', async () => {
    ...
    await expect(fetcherJsonp('/api/404', {
      timeout: 5000
    })).rejects.toThrow('fetcher-jsonp network failure, url = /api/404');
  ...
  test('unreachable url will timeout', async () => {
    ...
    await expect(fetcherJsonp(JSONP_URL, {
      timeout: 100
    })).rejects.toThrow(`fetcher-jsonp timeout, url = ${JSONP_URL}, timeout = 100ms`);
  ...
  test('custom callback, lifecycle and charset (success)', async () => {
    ...
    // @ts-expect-error accessing window as any for jsonp callback in jsdom test
    ...
    // @ts-expect-error callback should be cleared after promise resolves
  ...
  test('custom callback, lifecycle and charset (failure - network error)', async () => {
    ...
    expect(await promise.catch((e: unknown) => e as Error)).toHaveProperty('name', 'JsonpError.Network');
    
    // @ts-expect-error callback cleared on network error
  ...
  test('custom callback, lifecycle and charset (failure - timeout)', async () => {
    ...
    expect(await promise.catch((e: unknown) => e as Error)).toHaveProperty('name', 'JsonpError.Timeout');
    
    // by design: timeout 后 cleanupPrematurely 把 callback 设为 noop（仍 truthy）
    // @ts-expect-error callback becomes noop after timeout, not removed
    expect(window[callbackFn]).toBeTruthy();
    expect(document.querySelector(scriptQuery)).toBeFalsy(); // cleared
  });
  ...
  test('can be aborted', async () => {
    ...
    await expect(promise).rejects.toThrow('fetcher-jsonp abort, url = /api/abort');
